...
class CACRLearner(object):
        ...
        if mode=='train':
            self.optimizer = torch.optim.Adam(
                [{'params': self.model.encoder.parameters(), 'lr': 0.001},
                 {'params': self.model.ffcr_learner.parameters()}], lr=args.lr)
    ...
    def train(self, data):
        ...
        query_logits, loss = self.model(support_x, support_y, query_x, query_y, Aux=True)
    ...
    def test(self, data):
        ...
        with torch.no_grad():
            logits, loss = self.model(support_x, support_y, query_x, query_y, Aux=False)
