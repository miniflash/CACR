...
class PointTransformerV3(PointModule):
    ...
    def forward(self, pc):
        ...
        """
        B, C, N = pc.shape
        point = pc.transpose(1, 2).contiguous().view(-1, C)
        grid = point
        grid_size = torch.tensor([0.02], dtype=torch.float32).to(point.device)
...
class PTV3_Seg(nn.Module):
    def __init__(self, num_classes):
        super(PTV3_Seg, self).__init__()
        self.encoder = PointTransformerV3(in_channels=3, enc_depths=(2, 2, 2, 6, 2),
                                          enc_channels=(32, 64, 128, 256, 512), dec_depths=(2, 2, 2, 2),
                                          dec_channels=(64, 64, 128, 256))
        self.segmenter = nn.Conv1d(64, num_classes, 1, 1, 0)

    def forward(self, pc):
        B, C, N = pc.shape
        point_feat = self.encoder(pc)
        x = self.segmenter(point_feat["feat"].view(B, N, -1).permute(0, 2, 1))
        return x